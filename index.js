...
const totalContributions = GAMES_JSON.reduce((contribution, game) =>{
    return contribution + game.backers;
}, 0)

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `
        <p class="card-subtitle">${totalContributions.toLocaleString('en-US')}</p>
    `;
    

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const totalRaised = GAMES_JSON.reduce((raisedAmount, game) => {
    return raisedAmount + game.pledged;
}, 0);

// set inner HTML using template literal
raisedCard.innerHTML = `
        <p class="card-subtitle">$${totalRaised.toLocaleString('en-US')}</p>
    `;
...
gamesCard.innerHTML = `
        <p class="card-subtitle">${totalGames.toLocaleString('en-US')}</p>
    `; 
...
searchInput.addEventListener("input", fixSearchInput);



// Add a function to filter games based on the search input
...
function fixSearchInput() {
    ...
    // Filter games that start with the search term
    const filteredGames = GAMES_JSON.filter(game => 
        game.name.toLowerCase().startsWith(searchTerm)
    );
    
    // Clear the container and add filtered games
    deleteChildElements(gamesContainer);
